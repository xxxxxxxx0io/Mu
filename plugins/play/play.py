...
@app.on_message(
    command(["play", "تشغيل", "vplay", "cplay", "cvplay", "playforce", "bgtforce", "vplayforce", "cplayforce", "cvplayforce"])
    & filters.group
    & ~filters.edited
    & ~BANNED_USERS
)
...
async def play_commnd(
    ...
    mystic = await message.reply_text("**🏷وضع تشغيل القناه**\n\n**🔄 جٍآﮯيَ ...**\n**آلقنْآہ آلُمتٌصٍلُہ:** {0}".format(channel) if channel else "**🔄 آنتظـر لحظـهہ‏‏ ...**")
    plist_id = None
    slider = None
    plist_type = None
    spotify = None
    ...
    if audio_telegram:
        if audio_telegram.file_size > config.TG_AUDIO_FILESIZE_LIMIT:
            return await mystic.edit_text("**❌ فشـل فيـﮯ تشـغيـﮯل آلمـلف آلصـوتيـﮯ**\n\n**لزمـ يـﮯمـون آقل مـن ١٠٠ مـيـﮯجآ**")
        duration_min = seconds_to_min(audio_telegram.duration)
        if (audio_telegram.duration) > config.DURATION_LIMIT:
            return await mystic.edit_text(
                "**🤖 خطـآ فيـﮯ مـدهہ‏‏ آلتشـغيـﮯل❗**\n\n**آلوقت آلمـسـمـوح: **{0} دقآيـﮯق(𝐬)\n**آلوقت:** {1} آلسـآعهہ‏‏(𝐬)".format(
        ...
        if not await is_video_allowed(message.chat.id):
            return await mystic.edit_text("**❌ فشـغل ف آلآحضـآر ✨...**")
        ...
        if await YouTube.exists(url):
            if "playlist" in url:
                try:
                    ...
                    print(e)
                    return await mystic.edit_text("**❌ حدث خطـآ ف آلتشـغيـﮯل ✨...**")
                streamtype = "playlist"
                plist_type = "yt"
                if "&" in url:
                ...
                cap = "**💿 قآيـﮯمـهہ‏‏ آليـﮯوتيـﮯب**\n\n🌺 آختآر آغنيـﮯتگ. آلمـفضـلهہ‏‏✨ ...**"
            else:
                try:
            ...
            try:
                ...
                return await mystic.edit_text(
                    "**🥀 🥀 خطـآ ف آلتشـغيـﮯل.\n\n❗ آلنوع:-** `{0}`".format(type(e).__name__)
                )
    ...
    if CallbackQuery.from_user.id != int(user_id):
        try:
            return await CallbackQuery.answer(
                "🥀 هہ‏‏ذهہ‏‏ آلآغنيـﮯهہ‏‏ ليـﮯسـت لگ❗ آبحث عن آغنيـﮯتگ آلمـفضـلهہ‏‏  ✨ ...", show_alert=True
            )
        except:
            return
    try:
...
async def anonymous_check(client, CallbackQuery):
    try:
        await CallbackQuery.answer(
            "🤖 هہ‏‏نآگ خطـآ آنت مـشـرف مـتخفيـﮯ قمـ بتعديـﮯل آلصـلآحيـﮯآت❗",
            show_alert=True,
        )
    except:
        return
...
async def slider_queries(client, CallbackQuery, _):
    ...
    if what == "F":
        ...
        try:
            await CallbackQuery.answer("🔃 جآريـﮯ آلتخطـيـﮯ🌷 ...")
        except:
            pass
    ...
    if what == "B":
        if rtype == 0:
            ...
            query_type = int(rtype - 1)
        try:
            await CallbackQuery.answer("🔃 خآريـﮯ آلتخطـيـﮯ 🌷 ...")
        except:
            pass
        title, duration_min, thumbnail, vidid = await YouTube.slider(
            query, query_type
